...
def plot_gc():
    ...
    #             'GGA', 'GGG', 'GGT', 'GGC', 'ATC']
    codons = ['CGG', 'CGT', 'CGC', 'GCA', 'GCG', 'GCT', 'GCC', 'TCT', 'TCC', 'TTA', 'TTG',
              'CTA', 'CTG', 'ACA', 'ACG', 'CTT', 'CTC', 'GTT', 'GTC', 'CCA', 'CCG', 'CCT', 'CCC', 'GTA',
              'GTG', 'TAA', 'TAG', 'TAT']

    # codons = ['AAA', str(Seq('AAA').reverse_complement())]
    non_codons = ['NNN', 'NNG', 'NNC', 'NNA', 'NNT', 'NGG', 'NGC', 'NGA', 'NGT', 'NCC', 'NCG', 'NCA', 'NCT',
                  ...
                  'TCN', 'TAN', 'AAT', 'AAC', 'GAA', 'GAG', 'TTT', 'TTC', 'TAC', 'AAA', 'AAG', 'GAT', 'GAC',
                  'ATG', 'ATT', 'ACT', 'TGA', 'TGG', 'TGT', 'TGC', 'ACC', 'ATA', 'CAA', 'CAG', 'CAT', 'CAC',
                  'AGA', 'AGG', 'AGT', 'AGC', 'TCA', 'TCG', 'GGA', 'GGG', 'GGT', 'GGC', 'ATC', 'CGA', 'CGG']
            ...
            if values[0] in names:  # If the fasta header is present in the names list it should be included in the calculations
                ...
                for idx, data in enumerate(pd.read_csv(
                    ...
                    for interval in list(range(0, len(data), window_size)):  # Window size 1000 bp
                        ...
                        slider.fit(np.array(list(values[1][telomeres[chr_nr]+idx*chunk_size+start:telomeres[chr_nr]+idx*chunk_size+stop])))
